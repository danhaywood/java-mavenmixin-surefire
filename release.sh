...
echo ""
mvn clean deploy -Pdanhaywood-mavenmixin-surefire -Dskip.danhaywood-mavenmixin-surefire -Dpgp.secretkey=keyring:id=$KEYID -Dpgp.passphrase="literal:$PASSPHRASE"
if [ $? != 0 ]; then
    echo "... failed" >&2
    exit 1
fi
