...
* `mvn verify` +
+
Runs the unit tests (because `test` phase precedes `verify` phase) and also the integration tests and Cucumber BDD specs.
Integration tests must match the name `\*IntegTest*.java`.
link:https://cucumber.io/docs/reference/jvm#running-cucumber[BDD specs] must match the name `\*IntegBddSpecs.java`. +
+
[TIP]
====
